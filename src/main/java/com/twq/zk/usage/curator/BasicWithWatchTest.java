...
public class BasicWithWatchTest {
    ...
    public static class DataGetterWatch implements Watcher {
        ...
        public void process(WatchedEvent event) {
            try {
                    System.out.println(new String(client.getData().usingWatcher(new DataGetterWatch()).forPath(event.getPath())));
                } catch (Exception e) {
                    e.printStackTrace();
            }
        }
    ...
    public static class ChildrenGetterWatch implements Watcher {

        @Override
        public void process(WatchedEvent event) {
            try {
                System.out.println(client.getChildren().usingWatcher(new DataGetterWatch()).forPath(event.getPath()));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
